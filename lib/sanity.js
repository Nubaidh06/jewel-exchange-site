...
export async function getRelatedProducts(type, category, currentSlug, limit = 8) {
  try {
    const rawItems = await client.fetch(
      `*[_type == "product" && (type == $type || catalog_type == $type) && slug.current != $currentSlug] | order((category == $category) desc, name asc)[0...$limit] {
        _id,
        name,
        "slug": slug.current,
        type,
...
export async function getShowcaseItems() {
  try {
    const rawItems = await client.fetch(`*[_type == "product"] | order(_updatedAt desc) {
      _id,
      name,
      "slug": slug.current,
      type,
    ...
    if (Array.isArray(rawItems) && rawItems.length > 0) {
      return rawItems.map((item) => {
        const mainImg = formatSanityImage(item.imgObj, 1000) || (item.rawImgUrl ? `${item.rawImgUrl}?auto=format&w=1000` : null);
        const gallery = Array.isArray(item.galleryObjs) && item.galleryObjs.length > 0
          ? item.galleryObjs.map((g, idx) => formatSanityImage(g, 1000) || (item.galleryUrls?.[idx] ? `${item.galleryUrls[idx]}?auto=format&w=1000` : null)).filter(Boolean)
          : Array.isArray(item.galleryUrls)
          ? item.galleryUrls.map((u) => `${u}?auto=format&w=1000`)
          : [];
        return {
          ...item,
          img: mainImg,
          gallery,
          images: mainImg ? [mainImg, ...gallery] : [...gallery],
        };
      });
    }
  } catch (error) {
    console.error("Error fetching showcase items from Sanity, falling back to local dataset:", error);
  }

  // Robust fallback to local dataset if Sanity fails
  try {
    const fallback = await import('./fallback_products.json', { assert: { type: 'json' } }).catch(() => null);
    if (fallback && Array.isArray(fallback.default || fallback)) {
      const list = fallback.default || fallback;
    ...
    console.warn("Could not load fallback_products.json:", e);
  }
  return [];
}
