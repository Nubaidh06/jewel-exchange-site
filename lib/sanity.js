...
export async function getJewelry() {
  try {
    return await client.fetch(`*[_type == "product" && (type == "Jewelry" || catalog_type == "Jewelry" || category in ["Rings", "Necklaces", "Earrings", "Bracelets", "Pendants"])] | order(_updatedAt desc) {
      _id,
      name,
      "slug": slug.current,
      type,
