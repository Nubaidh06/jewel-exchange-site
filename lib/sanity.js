...
export async function getProductBySlug(slug) {
  try {
    const raw = await client.fetch(`*[_type == "product" && slug.current == $slug][0] {
      _id,
      name,
      "slug": slug.current,
      type,
      ...
      description,
      specifications,
      "mainImage": coalesce(img.asset->url, image.asset->url),
      "galleryImages": coalesce(gallery[defined(asset)].asset->url, gallery[].asset->url, [])
    }`, { slug })

    if (!raw) return null;

    const img = raw.mainImage ? `${raw.mainImage}?auto=format&w=1200` : null;
    const gallery = Array.isArray(raw.galleryImages)
      ? raw.galleryImages.filter(Boolean).map((url) => `${url}?auto=format&w=1200`)
      : [];
