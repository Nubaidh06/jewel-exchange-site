...
export async function getJewelry() {
  try {
    return await client.fetch(`*[_type == "product" && (type == "Jewelry" || catalog_type == "Jewelry" || category in ["Rings", "Necklaces", "Earrings", "Bracelets", "Pendants"])] | order(_createdAt desc) {
      _id,
      name,
      "slug": slug.current,
      type,
...
export async function getGemstones() {
  try {
    return await client.fetch(`*[_type == "product" && (type == "Gemstones" || type == "Gemstone" || catalog_type == "Gemstones" || catalog_type == "Gemstone" || category in ["Sapphires", "Padparadscha", "Rubies", "Emeralds", "Diamonds", "Rare Gems", "Semi-Precious"])] | order(_createdAt desc) {
      _id,
      name,
      "slug": slug.current,
      type,
